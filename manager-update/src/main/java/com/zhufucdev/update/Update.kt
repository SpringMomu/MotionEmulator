...
class Updater(
    ...
    private val releaseResolver: (suspend (HttpClient) -> ReleaseAsset?)? = null
) {
    ...
    suspend fun check(currentVersion: String? = null): ReleaseAsset? {
        ...
        val update = if (releaseResolver != null) releaseResolver.invoke(ktor)
            else ktor.getReleaseAsset(apiUri, productAlias, "android", current, arch)
        this.update = update
        updateStatus(if (update != null) StatusReadyToDownload else StatusIdling)
        return update
    }
