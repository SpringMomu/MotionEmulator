...
class SettingsActivity : AppCompatActivity(),
    ...
    class MapsFragment : M3PreferenceFragment() {
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            val available = mutableListOf("osm" to "OpenStreetMap")
            if (BuildConfig.AMAP_SDK_KEY != "UNCONFIGURED") available.add("amap" to "AMap")
            if (BuildConfig.GCP_MAPS_KEY != "UNCONFIGURED") available.add("gcp_maps" to "Google Maps")
            for (key in listOf("map_provider", "poi_provider")) {
                findPreference<ListPreference>(key)?.apply {
                    entries = available.map { it.second }.toTypedArray()
                    entryValues = available.map { it.first }.toTypedArray()
                }
            }
        }
    }
